...
import { Tree, TreeViewer, useTree } from "./components/tree";
import { nodesPattern, nodesOnto, getRemoteData, nodesDocs, nodesStruct } from "./data";

export default function TreeView({step, hadleSelected} : {step:string, hadleSelected: (setted:string[])=>void}) {
  const nodes = step == "struct"? nodesStruct : step==="onto" ? nodesOnto :  step==="docs" ? nodesDocs : nodesPattern;
  ...
  return (
    <ChakraProvider>
      <Box p={5}>
        <TreeViewer value={nodes} isReadOnly={false} isDisabled={false} />
      </Box>
    </ChakraProvider>
  );
}
